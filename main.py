...
def main():
    ...
    while True:
        ...
        if inp == 9:
            ...
            if inp9 == 1:
                np.savetxt("coordinates15.csv", position_list, delimiter="$")  # Save the coordinates
                np.savetxt("data_genotypes15.csv", genotype_matrix, delimiter="$")  # Save the data 
                print("Saving Complete.")
                
            elif inp9 == 2:       
                position_list = np.loadtxt('./coordinates15.csv', delimiter='$').astype('float64')
                genotype_matrix = np.loadtxt('./data_genotypes15.csv', delimiter='$').astype('float64')
